...
| 0003 | `0003-add-env-gated-v4l2m2m-pacing-experiments.patch` | This repo | Adds runtime-only pacing experiment gates without changing defaults: `MOONLIGHT_V4L2M2M_PACING=prefer-low-delay` disables SDL present vsync for lower-delay measurement, while `MOONLIGHT_V4L2M2M_TIGHT_THRESHOLDS=1` drops display frames that have waited longer than `MOONLIGHT_V4L2M2M_TIGHT_LATE_US` before presentation. |
| 0004 | `0004-add-absolutetouch-flag-for-tap-to-click.patch` | This repo | Adds the `-absolutetouch` CLI flag (and matching `absolute_touch` config key). When enabled, touchscreens feed `LiSendMousePositionEvent` using the device's `ABS_X`/`ABS_Y` (fallback `ABS_MT_POSITION_X`/`ABS_MT_POSITION_Y`) min/max as the reference plane, with `-rotate` applied inside that plane. Tap-to-click anchors the synthesized click at the touchdown position so the host cursor jumps to the finger before pressing. Default is off, preserving upstream trackpad behavior; opted in by downstream product launchers for handhelds where the touchscreen drives GUI dialogs over the stream rather than acting as a trackpad. |

All four patches are in `manifest.patches`. 0002 was authored on Fuji in
