...
| 0004 | `0004-add-absolutetouch-flag-for-tap-to-click.patch` | This repo | Adds the `-absolutetouch` CLI flag (and matching `absolute_touch` config key). When enabled, touchscreens feed `LiSendMousePositionEvent` using the device's `ABS_X`/`ABS_Y` (fallback `ABS_MT_POSITION_X`/`ABS_MT_POSITION_Y`) min/max as the reference plane, with `-rotate` applied inside that plane. Tap-to-click anchors the synthesized click at the touchdown position so the host cursor jumps to the finger before pressing. Default is off, preserving upstream trackpad behavior; opted in by Korri's launcher for handhelds where the touchscreen drives GUI dialogs over the stream rather than acting as a trackpad. |
