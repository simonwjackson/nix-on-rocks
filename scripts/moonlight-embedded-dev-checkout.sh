...
# `nix-on-rocks-dev`. Patches can be edited normally; when done, run
#
#   git -C <checkout> format-patch <short-rev>..nix-on-rocks-dev \
...
# Resolve repo root by anchoring on the package manifest, walking up from
# this script's location. nix-on-rocks does not yet ship a top-level
# flake.nix; the monorepo merge restructure (docs/plans/2026-05-22-001-...)
# is the one that introduces it. Until then the manifest is the only stable
# anchor that identifies this repo's package surface.
script_dir=$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)
repo_root=$(CDPATH='' cd -- "$script_dir/.." && pwd)

manifest="$repo_root/packages/moonlight-embedded/manifest.nix"
[[ -f "$manifest" ]] \
  || fail "manifest not found: $manifest (expected at <repo>/packages/moonlight-embedded/manifest.nix)"
...
git -C "$checkout" branch --quiet -f nix-on-rocks-dev "$rev"
git -C "$checkout" checkout --quiet nix-on-rocks-dev
...
cat <<EOF
  ...
  Next steps:
    ...
    git -C $checkout format-patch $rev..nix-on-rocks-dev \\
