...
verifier="${repo_root}/scripts/verify-rk3326-payloads"

require_tool() {
  command -v "$1" >/dev/null 2>&1 || {
    echo "rk3326-payload-contract: SKIP missing $1" >&2
    exit 0
  }
}
...
make_payload() {
  ...
  local image="${target_dir}/ROCKNIX-RK3326-test-R36TMax.img"
  ...
  printf 'kernel\n' > "${target_dir}/KERNEL"
  printf 'system\n' > "${target_dir}/SYSTEM"
  printf 'dtb\n' > "${target_dir}/rk3326-gameconsole-r36tmax.dtb"
  case "${extlinux_state}" in
    valid)
      cat > "${target_dir}/extlinux.conf" <<'EOF'
LABEL ROCKNIX
  LINUX /KERNEL
  FDT /rk3326-gameconsole-r36tmax.dtb
  APPEND boot=${partition_boot} disk=${partition_storage} quiet
EOF
      ;;
    fdt-dir)
      cat > "${target_dir}/extlinux.conf" <<'EOF'
...
LABEL ROCKNIX
  LINUX /KERNEL
  FDT /rk3326-gameconsole-eeclone.dtb
  APPEND boot=LABEL=ROCKNIX disk=LABEL=STORAGE quiet
EOF
      ;;
    wrong-boot)
...
EOF
  ...
  if [ "${include_dtb}" = "yes" ]; then
    mcopy -i "${fat}" "${target_dir}/rk3326-gameconsole-r36tmax.dtb" ::/rk3326-gameconsole-r36tmax.dtb
  fi
  ...
  gzip -c "${image}" > "${target_dir}/ROCKNIX-RK3326-test-R36TMax.img.gz"
  if [ "${sha_state}" = "valid" ]; then
    (cd "${target_dir}" && sha256sum ROCKNIX-RK3326-test-R36TMax.img.gz > ROCKNIX-RK3326-test-R36TMax.img.gz.sha256)
  else
    printf '%064d  ROCKNIX-RK3326-test-R36TMax.img.gz\n' 0 > "${target_dir}/ROCKNIX-RK3326-test-R36TMax.img.gz.sha256"
  fi
...
expect_failure 'expected exactly one RK3326 SD image' "${verifier}" "${empty_dir}"
...
cp "${multi_image_dir}/ROCKNIX-RK3326-test-R36TMax.img.gz" "${multi_image_dir}/ROCKNIX-RK3326-test-b.img.gz"
(
  cd "${multi_image_dir}"
  sha256sum ROCKNIX-RK3326-test-b.img.gz > ROCKNIX-RK3326-test-b.img.gz.sha256
)
SUBDEVICE=R36TMax NIX_ON_ROCKS_BUILD_MANIFEST="${multi_image_dir}/manifest.md" "${verifier}" "${multi_image_dir}" >/dev/null

forbidden_fdt_dir=$(mktemp -d "${tmp_root}/fdt-dir.XXXXXX")
make_payload "${forbidden_fdt_dir}" 32768 ROCKNIX STORAGE valid fdt-dir
expect_failure 'must force /rk3326-gameconsole-r36tmax.dtb' "${verifier}" "${forbidden_fdt_dir}"

wrong_fdt_dir=$(mktemp -d "${tmp_root}/wrong-fdt.XXXXXX")
make_payload "${wrong_fdt_dir}" 32768 ROCKNIX STORAGE valid wrong-fdt
expect_failure 'must force /rk3326-gameconsole-r36tmax.dtb' "${verifier}" "${wrong_fdt_dir}"
...
expect_failure 'manifest must state R36T Max RK915 Wi-Fi hardware proof' \
...
expect_failure 'must boot by ROCKNIX FAT label or partition_boot placeholder' "${verifier}" "${wrong_boot_dir}"

wrong_disk_dir=$(mktemp -d "${tmp_root}/wrong-disk.XXXXXX")
make_payload "${wrong_disk_dir}" 32768 ROCKNIX STORAGE valid wrong-disk
expect_failure 'must mount storage by STORAGE label or partition_storage placeholder' "${verifier}" "${wrong_disk_dir}"

missing_dtb_dir=$(mktemp -d "${tmp_root}/missing-dtb.XXXXXX")
make_payload "${missing_dtb_dir}" 32768 ROCKNIX STORAGE valid valid no
expect_failure 'missing /rk3326-gameconsole-r36tmax.dtb' "${verifier}" "${missing_dtb_dir}"
...
printf 'rk3326-payload-contract: ok\n'
